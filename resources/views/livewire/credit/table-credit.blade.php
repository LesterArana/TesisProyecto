            ...
            <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tr>
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'employee.name', 'displayName' => 'Empleado' ])
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'amount', 'displayName' => 'Monto Total' ])
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'remaining_amount', 'displayName' => 'Saldo Restante' ])
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'installments', 'displayName' => 'Cuotas' ])
                        ...
                        @foreach ($credits as $credit)
                            <tr wire:key="{{ $credit->id }}" class="border-b">
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    {{ $credit->employee->person->name }}
                                </td>
                                <td class="px-4 py-3">Q {{ number_format($credit->amount, 2) }}</td>
                                <td class="px-4 py-3">Q {{ number_format($credit->remaining_amount, 2) }}</td>
                                <td class="px-4 py-3">{{ $credit->installments }}</td>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            Livewire.on('mostrarAlerta', creditId => {
                Swal.fire({
                    ...
                    text: "¡No podrás recuperar este crédito si tiene pagos asociados!",
                    icon: "warning",

                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tr>
                            ...
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'address', 'displayName' => 'Dirección' ])
                        ...
                        @foreach ($projets as $projet)
                            <tr wire:key="{{ $projet->id }}" class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    {{ $projet->name }}
                                </th>
                                <td class="px-4 py-3">{{ $projet->address }}</td>
                                <td class="px-4 py-3">{{ $projet->status ? 'Activo' : 'Inactivo' }}</td>
                                <td class="px-4 py-3">{{ $projet->created_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="flex space-x-1">
                                        <a href="{{ route('projets.show', $projet->id) }}" class="bg-green-400 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Ver</a>
                                        <a href="{{ route('projets.edit', $projet->id) }}" class="bg-blue-800 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Editar</a>
                                        <button wire:click="$dispatch('mostrarAlerta',{{$projet->id}})"
...
@push("scripts")
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            Livewire.on('mostrarAlerta', projetId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteProjet', {projetId:projetId});
                    }
                });
            });
        });

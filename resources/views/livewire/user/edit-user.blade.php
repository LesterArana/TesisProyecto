<div class="flex justify-center my-6">
    <form class="md:w-1/2 space-y-5 p-9 bg-white rounded-lg shadow-md" wire:submit.prevent='update'>
        ...
        <div>
            ...
            <x-text-input id="password" class="block mt-1 w-full" type="password" wire:model="password" placeholder="Nueva Contraseña" />
        ...
        <div>
            ...
            <select id="role" wire:model="role"  class="block w-full p-2 border border-gray-300 rounded text-black bg-white">
                ...
                @foreach($roles as $role)
                    <option value="{{ $role->name }}" {{ $role->name === $this->role ? 'selected' : '' }}>{{ $role->name }}</option>
                @endforeach
            </select>

            ...
            <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tr>
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'person.name', 'displayName' => 'Nombre' ])
                            @include('livewire.includes.table-sortable-th',[ 'name' => 'position.name', 'displayName' => 'Puesto' ])
                        ...
                        @foreach ($employees as $employee)
                            <tr wire:key="{{ $employee->id }}" class="border-b">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="flex space-x-1">
                                            <a href="{{ route('employees.show', $employee->id) }}" class="bg-green-400 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Ver</a>
                                            <a href="{{ route('employees.edit', $employee->id) }}" class="bg-blue-800 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Editar</a>
                                            <button wire:click="$dispatch('mostrarAlerta', {{ $employee->id }})"
...
@push("scripts")
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            Livewire.on('mostrarAlerta', employeeId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deleteEmployee', {employeeId:employeeId});
                    }
                });
            });
        });

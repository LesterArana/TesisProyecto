                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        @foreach ($payment_methods as $payment_method)
                            <tr wire:key="{{ $payment_method->id }}" class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    {{ $payment_method->name }}
                                </th>
                                <td class="px-4 py-3">{{ $payment_method->created_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="flex space-x-1">
                                        @can('payment_methods.show')
                                            <a href="{{ route('payment_methods.show', $payment_method->id) }}" class="bg-green-400 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Ver</a>
                                        @endcan
                                        @can('payment_methods.edit')
                                            <a href="{{ route('payment_methods.edit', $payment_method->id) }}" class="bg-blue-800 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">Editar</a>
                                        @endcan
                                        @can('payment_methods.destroy')
                                            <button wire:click="$dispatch('mostrarAlerta',{{$payment_method->id}})"
...
@push("scripts")
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            Livewire.on('mostrarAlerta', paymentMethodId => {
                Swal.fire({
                    title: "¿Eliminar Método de Pago?",
                    text: "¡No podrás recuperar este método de pago!",
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        Livewire.dispatch('deletePaymentMethod', { paymentMethodId:paymentMethodId});
                        ($destinationPlantId)
                    }
                });
            });

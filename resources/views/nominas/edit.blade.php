    ...
    <form action="{{ route('nominas.update', $nomina->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="tipo_pago" name="tipo_pago" required onchange="togglePagoFields()">
                <option value="hora" {{ $nomina->tipo_pago == 'hora' ? 'selected' : '' }}>Por Hora</option>
                <option value="dia" {{ $nomina->tipo_pago == 'dia' ? 'selected' : '' }}>Por Día</option>
        ...
        <div id="horas_trabajadas_field" class="mb-3" style="display: none;">
            ...
            <select name="deducciones[]" class="form-control" multiple>
                @foreach($deducciones as $deduccion)
                    <option value="{{ $deduccion->id }}" {{ in_array($deduccion->id, $nomina->deducciones->pluck('id')->toArray()) ? 'selected' : '' }}>
                        {{ $deduccion->descripcion }} - @if($deduccion->es_porcentaje){{ $deduccion->monto }}%@else${{ $deduccion->monto }}@endif
                    </option>
                @endforeach
            </select>
        </div>
